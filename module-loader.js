(()=>{
  ...
  const session=parse(sessionStorage,'jarvis-session',null);
  ...
  function auditDenied(){
    ...
    log.unshift({id:crypto.randomUUID?.()||String(Date.now()),at:new Date().toISOString(),userId:user?.id||'guest',userName:user?.name||'Utilisateur non connecté',role:user?.role||'guest',module:moduleId,action:'access.denied',recordId:null,title:'Accès refusé'});
