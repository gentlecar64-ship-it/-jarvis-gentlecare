...
const store = {
  ...
  create(collection, input) {
    ...
    if (collection === 'quoteRequests') record.number = `DD-2026-${String(++requestNumber).padStart(4, '0')}`;
...
try {
  ...
  const autoProcedure = procedures.get('automobile');
  const motoProcedure = procedures.get('moto');
  assert.match(autoProcedure.label, /automobile/i);
  assert.match(autoProcedure.checklist.join(' '), /quatre points de levage/i);
  ...
  assert.equal(catalog.some((item) => /club|fondateur|pass/i.test(`${item.key} ${item.label}`)), false);
  ...
  const unsafeMargin = tariffs.margin({ standardPriceTtc: 1000, finalPriceTtc: 500, directCostTtc: 700, targetMarginPercent: 30 });
  assert.ok(unsafeMargin.warnings.length >= 1);

  const draft = quoteRequests.saveDraft(store, {
    clientName: 'Client Moto', email: 'moto@example.com', mobile: '0611223344',
    vehicleType: 'moto', customerType: 'particulier', brand: 'Ducati', model: 'Monster', registration: 'MO-123-TO', year: 2021,
    tariffKey: 'moto-particulier', finalPrice: 900, service: 'Cryonettoyage moto', voiceText: 'Ducati Monster 2021'
  }, technician);
  assert.ok(draft.request.number.startsWith('DD-2026-'));
  assert.ok(draft.client.id);
  assert.equal(draft.vehicle.vehicleType, 'moto');
  ...
  assert.equal(quoteRequests.list(store, technician).length, 1);
  assert.equal(quoteRequests.list(store, david).length, 1);
  ...
  store.create('interventions', { number: 'GC-2026-0001', service: 'Cryonettoyage automobile', scheduledDate: '2026-09-07', estimatedEndDate: '2026-09-08', status: 'Prévue' });
  const ics = calendar.buildIcs(store);
  assert.match(ics, /BEGIN:VCALENDAR/);
  assert.match(ics, /Cryonettoyage automobile/);
  assert.equal(calendar.tokenValid('mauvais-token'), false);

  console.log('Emergency, quote request, tariffs, procedures and calendar smoke test passed.');
} finally {
