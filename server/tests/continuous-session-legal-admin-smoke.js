...
class MemoryStore {
  constructor(seed = {}) {
    this.db = { clients:[], vehicles:[], quotes:[], quoteRequests:[], interventions:[], communications:[], tasks:[], documents:[], planningBlocks:[], externalCalendarEvents:[], leaveRequests:[], ...seed };
  }
...
function request(token) {
  return { url:'/api/auth/me', headers:{ authorization:`Bearer ${token}`, 'x-gcos-client':'pc', 'x-gcos-device-id':'test-pc', host:'localhost:4782', 'user-agent':'MAVIK smoke test' }, socket:{ remoteAddress:'127.0.0.1' } };
}
...
try {
  ...
  const saved = quoteRequests.saveDraft(store, {
    requestCategory:'voiture', clientName:'Client Exemple', email:'client@example.com', brand:'Ford', model:'Mustang', registration:'AA-123-BB',
    service:'Pack automobile particulier', targetPrice:1500, finalPrice:1500,
    termsAccepted:true, termsAcceptedBy:'Client Exemple', technicalMediaAuthorized:true, expertTransmissionAuthorized:true,
    commercialMediaAuthorized:false, identifiableMediaAuthorized:false, emailAllowed:true, smsAllowed:false
  }, admin);
  ...
  console.log('Continuous session, legal quote and direction controls smoke test passed.');
} finally {
  fs.mkdirSync(dataDir, { recursive:true });
