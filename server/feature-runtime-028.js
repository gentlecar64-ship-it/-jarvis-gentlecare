...
const FEATURE_KEYS = [
  'photoUrls','photoAnalysis','photoAnalysisConfirmed','dirtLevel','dirtyAreas','accessConstraints',
  'industrialSite','industrialMachineFunction','industrialDimensions','industrialMaterials','industrialZones','industrialEnergySources','industrialConsignation','industrialProductionConstraints','industrialAccessMeans','industrialWasteRecovery','industrialSafetyRules','industrialShutdownWindow',
...
function extraFields(input = {}) {
  ...
  return {
    ...
    industrialSite:text(input.industrialSite), industrialMachineFunction:text(input.industrialMachineFunction), industrialDimensions:text(input.industrialDimensions), industrialMaterials:text(input.industrialMaterials), industrialZones:text(input.industrialZones), industrialEnergySources:text(input.industrialEnergySources), industrialConsignation:text(input.industrialConsignation), industrialProductionConstraints:text(input.industrialProductionConstraints), industrialAccessMeans:text(input.industrialAccessMeans), industrialWasteRecovery:text(input.industrialWasteRecovery), industrialSafetyRules:text(input.industrialSafetyRules), industrialShutdownWindow:text(input.industrialShutdownWindow),
    deliveryRequired, deliveryDestination:text(input.deliveryDestination || 'Bayonne'), deliveryTrips, deliveryRateHt, deliveryAmountHt, deliveryAmountTtc, deliveryAssignee:deliveryRequired ? DELIVERY_ASSIGNEE : '', deliveryVehicle:deliveryRequired ? DELIVERY_RESOURCE : '', deliveryStartTime:DELIVERY_START, deliveryEndTime:DELIVERY_END,
    estimatedDryIceKg:number(input.estimatedDryIceKg)
  };
}
function featureText(extras, category) {
  const lines = ['', 'PHOTOGRAPHIES ET CONSTAT DE VISITE'];
  lines.push(`Photographies enregistrées : ${extras.photoUrls.length}`);
  lines.push(`Identification visuelle confirmée par l’utilisateur : ${extras.photoAnalysisConfirmed ? 'OUI' : 'NON'}`);
  ...
  if (category === 'industriel') {
    ...
    const records = [
      ['Entreprise et site',extras.industrialSite],['Machine et fonction',extras.industrialMachineFunction],['Dimensions / poids',extras.industrialDimensions],['Matériaux / revêtements',extras.industrialMaterials],['Zones et résultat attendu',extras.industrialZones],['Énergies et organes sensibles',extras.industrialEnergySources],['Consignation',extras.industrialConsignation],['Contraintes de production',extras.industrialProductionConstraints],['Accès / levage / balisage',extras.industrialAccessMeans],['Ventilation / confinement / déchets',extras.industrialWasteRecovery],['Règles de sécurité et documents',extras.industrialSafetyRules],['Fenêtre d’arrêt',extras.industrialShutdownWindow]
    ];
    for (const [label,value] of records) lines.push(`${label} : ${value || 'À compléter avant validation définitive'}`);
  }
  if (extras.deliveryRequired) {
    lines.push('', 'SERVICE LIVRAISON');
    lines.push(`Destination : ${extras.deliveryDestination || 'Bayonne'}`);
    lines.push(`Nombre de voyages : ${extras.deliveryTrips}`);
    lines.push(`Tarif : ${euro(extras.deliveryRateHt)} HT par heure et par voyage`);
    lines.push(`Montant livraison : ${euro(extras.deliveryAmountHt)} HT — ${euro(extras.deliveryAmountTtc)} TTC`);
    lines.push(`Planning ressource : ${extras.deliveryAssignee} · ${extras.deliveryVehicle} · ${extras.deliveryStartTime}–${extras.deliveryEndTime} par voyage`);
  }
  return lines.join('\n');
}
function preparedQuoteInput(input = {}) {
  ...
  return {
    ...
    serviceBaseTtc, finalPrice:totalTtc || number(input.finalPrice), customPrice:totalTtc || number(input.customPrice), targetPrice:totalTtc || number(input.targetPrice),
...
function patchVisual(visualUrl, extras) {
  ...
  const details=[];
  details.push(`${extras.photoUrls.length} photo(s) de visite conservée(s)`);
  ...
  const insert=`<text x="72" y="1860" font-size="14" fill="#9bd9ef" font-weight="800">PHOTO-FIRST MAVIK 0.28 — ${details.join(' · ')}</text>`;
  svg=svg.replace('</svg>',`${insert}</svg>`); fs.writeFileSync(target,svg,'utf8');
}
function installQuotePatches() {
  ...
  if(!quoteStudio.__mavikPhotoDeliveryPatched){
    ...
    quoteStudio.preview=function previewPhotoDelivery(store,input={},user={}){
      ...
      result.data.industrial=prepared.category==='industriel'?Object.fromEntries(FEATURE_KEYS.filter((key)=>key.startsWith('industrial')).map((key)=>[key,prepared[key]])):null;
    ...
    quoteStudio.confirm=function confirmPhotoDelivery(store,input={},user={}){
      ...
      const baseLine={label:result.quote.service||prepared.service||'Prestation',quantity:1,totalTtc:prepared.serviceBaseTtc};
      const lines=[baseLine]; if(extras.deliveryRequired)lines.push({label:`Service livraison ${extras.deliveryDestination} — ${extras.deliveryTrips} voyage(s)`,quantity:extras.deliveryTrips,unitPriceHt:DELIVERY_RATE_HT,totalHt:extras.deliveryAmountHt,totalTtc:extras.deliveryAmountTtc});
      ...
      const vehicle=store.update('vehicles',result.vehicle.id,{photoUrl:extras.photoUrls[0]||result.vehicle.photoUrl,photoUrls:extras.photoUrls,photoAnalysis:extras.photoAnalysis,dirtLevel:extras.dirtLevel,dirtyAreas:extras.dirtyAreas,accessConstraints:extras.accessConstraints,...Object.fromEntries(FEATURE_KEYS.filter((key)=>key.startsWith('industrial')).map((key)=>[key,prepared[key]]))});
  ...
  if(!quoteRequests.__mavikPhotoDeliveryPatched){
    ...
    quoteRequests.saveDraft=function saveFeatureDraft(store,input={},user={}){const result=originalSave(store,input,user);const extras=extraFields(input);const request=store.update('quoteRequests',result.request.id,{...extras,photoUrl:text(input.photoUrl||extras.photoUrls[0]),serviceBaseTtc:number(input.serviceBaseTtc||input.finalPrice||input.targetPrice),featureVersion:'0.28'});if(result.vehicle?.id&&(extras.photoUrls.length||Object.values(extras).some(Boolean)))store.update('vehicles',result.vehicle.id,{photoUrl:extras.photoUrls[0]||result.vehicle.photoUrl,photoUrls:extras.photoUrls,photoAnalysis:extras.photoAnalysis,dirtLevel:extras.dirtLevel,dirtyAreas:extras.dirtyAreas,...Object.fromEntries(FEATURE_KEYS.filter((key)=>key.startsWith('industrial')).map((key)=>[key,input[key]]))});return{...result,request};};
    quoteRequests.submit=function submitFeatureRequest(store,input={},user={}){const result=originalSubmit(store,input,user);const extras=extraFields(input);const proposal=quoteStudio.preview(store,{...input,clientId:result.client?.id||input.clientId,vehicleId:result.vehicle?.id||input.vehicleId},user);const request=store.update('quoteRequests',result.request.id,{...extras,photoUrl:text(input.photoUrl||extras.photoUrls[0]),jarvisProposal:proposal,jarvisQuoteText:proposal.quoteText,featureVersion:'0.28'});return{...result,request,proposal};};
    quoteRequests.__mavikPhotoDeliveryPatched=true;
  }
  const jarvis=require('./jarvis-extended');
  if(!jarvis.__mavikPhotoQuotePatched){const original=jarvis.execute.bind(jarvis);jarvis.execute=function executePhotoQuote(store,input={}){const command=text(input.text||input.command).toLowerCase();if(/^(faire |créer |ouvrir )?(un )?devis[.! ]*$/.test(command)||/commence(r)? (un )?devis/.test(command))return{type:'open-photo-quote',answer:'J’ouvre le devis. Commencez par prendre une photo générale, puis ajoutez les zones sales ou à traiter.',links:[{label:'Ouvrir le devis photo',url:'/quotes?photo=1'}]};return original(store,input);};jarvis.__mavikPhotoQuotePatched=true;}
}
...
function navigationFeatureScript() {
...
function quoteFetchAugmentScript() {
...
function installHttpPatch() {
  ...
  http.createServer=function createFeatureServer(listener){return originalCreateServer(async(req,res)=>{const url=new URL(req.url,`http://${req.headers.host||'localhost'}`);try{
    ...
    if(req.method==='GET'&&url.pathname.startsWith('/feature-assets/')){const file=path.basename(url.pathname);const allowed=new Set(['workshop-day-client.js']);if(!allowed.has(file))return json(res,404,{error:'FEATURE_ASSET_NOT_FOUND'});return js(res,fs.readFileSync(path.join(PUBLIC_DIR,file),'utf8'));}
    ...
    if(url.pathname.startsWith('/api/planning/')&&['/api/planning/overview','/api/planning/propose','/api/planning/schedule','/api/planning/blocks'].includes(url.pathname)){const user=requireUser(req);requireDirection(user);if(req.method==='POST'&&url.pathname==='/api/planning/schedule'){const body=await readBody(req);return json(res,200,require('./planning-service').scheduleQuote(require('./local-store'),body,user));}}
    if(req.method==='POST'&&url.pathname==='/api/planning/reschedule-email'){const user=requireUser(req);return json(res,200,rescheduleFromClientEmail(require('./local-store'),await readBody(req),user));}
    if(req.method==='GET'&&url.pathname==='/api/workshop-day'){const user=requireUser(req);return json(res,200,require('./workshop-day-plan').build(require('./local-store'),{date:url.searchParams.get('date'),employeeName:url.searchParams.get('employeeName')},user));}
  ...
  }catch(error){return json(res,error.status||500,{error:error.message||'MAVIK_FEATURE_ERROR',missingFields:error.missingFields||undefined,conflicts:error.conflicts||undefined});}});};http.__mavikFeature028Patched=true;
}
