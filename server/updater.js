...
function runGit(args, options = {}) {
  return execFileSync('git', args, { cwd: ROOT_DIR, encoding: 'utf8', windowsHide: true, timeout: Number(options.timeout || 30000), stdio: options.stdio || ['ignore', 'pipe', 'pipe'] }).trim();
}
...
function gitStatus() {
  try { return { ok: true, version: runGit(['--version']), branch: updateBranch(), commit: localCommit(), remote: runGit(['remote', 'get-url', 'origin']) }; }
  catch (error) { return { ok: false, error: String(error.message || error), branch: updateBranch(), commit: localCommit() }; }
}
function state() {
  const saved = readJson(STATE_FILE, {});
  return { ...saved, enabled: process.env.GCOS_AUTO_UPDATE !== 'false', automaticInstall: process.env.GCOS_AUTO_INSTALL !== 'false', channel: process.env.GCOS_UPDATE_CHANNEL || 'development', branch: updateBranch(), currentVersion: currentVersion(), currentCommit: localCommit(), checking: checkingNow, installing: automaticUpdateRunning, updateAvailable: Boolean(saved.updateAvailable), pendingRestart: fs.existsSync(PENDING_FILE), git: gitStatus() };
}
...
async function branchMetadata() {
  const repository = process.env.GCOS_UPDATE_REPOSITORY || DEFAULT_REPOSITORY; const branch = updateBranch();
...
async function releaseMetadata() {
  ...
  if (!response.ok) throw new Error(`UPDATE_METADATA_${response.status}`);
  const release = await response.json(); const asset = (release.assets || []).find((item) => /gcos.*\.zip$/i.test(item.name)) || (release.assets || [])[0];
...
async function check() {
  ...
  try {
    const channel = process.env.GCOS_UPDATE_CHANNEL || 'development'; const latest = channel === 'stable' ? await releaseMetadata() : await branchMetadata();
...
async function sha256(filePath) {
  return new Promise((resolve, reject) => { const hash = crypto.createHash('sha256'); const stream = fs.createReadStream(filePath); stream.on('error', reject); stream.on('data', (chunk) => hash.update(chunk)); stream.on('end', () => resolve(hash.digest('hex'))); });
}
...
function restartServer() {
  const entry = path.join(__dirname, 'server.js'); const helper = path.join(__dirname, 'restart-helper.js');
  const child = spawn(process.execPath, [helper, String(process.pid), entry, __dirname], { cwd: __dirname, detached: true, stdio: 'ignore', windowsHide: true, env: process.env });
  child.unref(); setTimeout(() => process.exit(0), 400).unref();
}
function validateInstalledFiles() {
  ...
  const required = [
    ...
    path.join(__dirname, 'public', 'quotes.html'), path.join(__dirname, 'public', 'planning.html'), path.join(__dirname, 'public', 'jarvis-quote.js'), path.join(__dirname, 'public', 'quote-studio-client.js'),
    path.join(__dirname, 'public', 'planning-client.js'), path.join(__dirname, 'public', 'navigation-enhancer.js'), path.join(__dirname, 'public', 'morale-client.js'), path.join(__dirname, 'assets', 'logo', '01.txt')
  ];
...
async function automaticCycle() {
  ...
  const checked = await check(); if (checked.updateAvailable && process.env.GCOS_AUTO_INSTALL !== 'false') return download(); return checked;
}
...
function startAutomaticChecks() {
  ...
  const execute = () => automaticCycle().then((result) => { if (result.lastError) console.warn(`[MAVIK UPDATE] ${result.lastError}`); }).catch((error) => saveState({ lastError: String(error.message || error) }));
