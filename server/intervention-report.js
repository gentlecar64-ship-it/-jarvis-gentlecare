...
function build(store, interventionReference, patch = {}, actor = {}) {
  ...
  const report = {
    ...
    evidence: {
      ...
      workshopProcedure: {
        key: text(intervention.workshopProcedureKey), label: text(intervention.workshopProcedure?.label), version: text(intervention.procedureVersion || intervention.workshopProcedure?.version),
        status: text(intervention.procedureStatus), approvedAt: text(intervention.finalApprovedAt), approvedBy: text(intervention.finalApprovedByName),
        steps: list(intervention.procedureSteps).map((step) => ({ id: text(step.id), stage: text(step.stage), label: text(step.label), status: text(step.status), note: text(step.note), completedAt: text(step.completedAt), completedBy: text(step.completedByName), evidenceCount: list(step.evidence).length }))
      },
...
function renderHtml(report) {
  ...
  ${section(11, 'Pièces, procédure et preuves', `<div class="evidence"><p><strong>Devis :</strong> ${escapeHtml(report.evidence.quote?.number || 'Non rattaché')} · ${escapeHtml(report.evidence.quote?.status || '')}</p><p><strong>Procédure :</strong> ${escapeHtml(report.evidence.workshopProcedure.label || report.evidence.workshopProcedure.key || 'Non rattachée')} · v${escapeHtml(report.evidence.workshopProcedure.version || '—')} · ${escapeHtml(report.evidence.workshopProcedure.status || 'Non validée')}</p><p><strong>Étapes tracées :</strong> ${report.evidence.workshopProcedure.steps.length} · <strong>Médias :</strong> ${report.evidence.media.length} · <strong>Documents liés :</strong> ${report.evidence.relatedDocuments.length} · <strong>Fiches produits :</strong> ${report.evidence.productSheets.length}</p><h3>Exécution de la procédure</h3>${renderList(report.evidence.workshopProcedure.steps.map((step) => `${step.status === 'Terminée' ? '✓' : '○'} ${step.id} — ${step.label}${step.completedBy ? ` · ${step.completedBy}` : ''}${step.evidenceCount ? ` · ${step.evidenceCount} preuve(s)` : ''}`), 'Aucune étape de procédure rattachée.')}<h3>Historique des versions</h3>${renderList(report.evidence.versionHistory.map((item) => `v${item.version} · ${item.generatedAt || ''} · ${item.status || ''} · ${item.hash || ''}`), 'Première version du rapport.')}<p class="hash"><strong>Empreinte SHA-256 de cette version :</strong> ${escapeHtml(report.contentHash)}</p></div>`)}
...
function validate(store, interventionReference, input = {}, actor = {}) {
  ...
  const preview = build(store, intervention, { ...input, managerValidation: input.managerValidation || actor.name, validatedAt: iso() }, actor).report;
  if (!preview.completeness.complete) throw Object.assign(new Error('REPORT_INCOMPLETE'), { status: 409, missingFields: preview.completeness.missing });
