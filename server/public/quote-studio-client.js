(() => {
  ...
  const fieldIds = [
    ...
    'termsAcceptedBy','consentNotes'
  ];
  const checkboxIds = ['termsAccepted','technicalMediaAuthorized','expertTransmissionAuthorized','commercialMediaAuthorized','identifiableMediaAuthorized','emailAllowed','smsAllowed'];
  ...
  function setChecked(id, next) { const element = $(id); if (element) element.checked = next === true || next === 'true' || next === 'on' || next === 1; }
  ...
  function payload() {
    ...
    const termsAccepted = checked('termsAccepted');
    return {
      ...
      termsAccepted, termsAcceptedAt:termsAccepted ? new Date().toISOString() : '', termsAcceptedBy:value('termsAcceptedBy') || value('clientName'),
      technicalMediaAuthorized:checked('technicalMediaAuthorized'), expertTransmissionAuthorized:checked('expertTransmissionAuthorized'),
      commercialMediaAuthorized:checked('commercialMediaAuthorized'), identifiableMediaAuthorized:checked('identifiableMediaAuthorized'),
      emailAllowed:checked('emailAllowed'), smsAllowed:checked('smsAllowed'), consentNotes:value('consentNotes')
    };
  }
  ...
  function updateLegalStatus() {
    ...
    if (checked('termsAccepted')) {
      ...
      status.innerHTML = `<strong>CGV acceptées.</strong><div>Preuve préparée au nom de ${esc(value('termsAcceptedBy') || value('clientName') || 'client à confirmer')}. Les autorisations d’image restent indépendantes.</div>`;
    } else {
      status.className = 'status';
      status.textContent = 'CGV non acceptées — le devis peut être préparé, mais l’acceptation reste à recueillir avant l’engagement définitif.';
    }
  }
  ...
  function markChanged() {
    ...
    window.dispatchEvent(new CustomEvent('mavik-quote-change', { detail: payload() }));
  }
  function applyRole() {
  ...
  function startDictation() {
    ...
    recognition.onresult = (event) => { let transcript=''; for (let index=0; index<event.results.length; index+=1) transcript += `${event.results[index][0]?.transcript || ''} `; $('voiceText').value=transcript.trim(); state.acquired=true; window.dispatchEvent(new CustomEvent('mavik-quote-change',{detail:payload()})); };
  ...
  function selectClient(record) {
    state.selectedClient=record.client; state.selectedVehicle=null; set('clientId',record.client.id,true); set('clientName',record.client.name,true); set('mobile',record.client.mobile || record.client.phone,true); set('email',record.client.email,true); set('preferredChannel',record.client.preferredChannel || 'E-mail',true); set('address',record.client.address,true); setChecked('emailAllowed',record.client.emailAllowed); setChecked('smsAllowed',record.client.smsAllowed);
  ...
  function newDossier() {
    state.selectedClient=null; state.selectedVehicle=null; state.requestId='';
    fieldIds.forEach((id)=>{ if (!['preferredChannel','customerType','durationDays','expertReviewStatus','earliestDate'].includes(id)) set(id,'',true); });
    checkboxIds.forEach((id)=>setChecked(id,false));
    set('vehicleType','',true); set('durationDays','1',true); $('knownVehicles').innerHTML=''; $('selectedDossier').textContent='Nouveau dossier : chaque information saisie sera enregistrée automatiquement, même sans devis validé.'; state.dirty=false; state.acquired=false; state.specialOfferEnabled=false; $('specialOfferPanel').classList.remove('show'); updateCategoryButtons(); renderProcedure(); setSaveStatus('Nouvelle demande vide',false); resetValidation(); updateMetrics(); $('jarvisStatus').textContent='Jarvis : de quoi s’agit-il ?'; window.dispatchEvent(new CustomEvent('mavik-quote-change',{detail:payload()}));
  }
  ...
  function requestCard(request) {
    ...
    return `<article class="result-card"><strong>${esc(request.number || 'Demande')} — ${esc(request.clientName || 'Client à compléter')} — ${esc([request.brand,request.model].filter(Boolean).join(' ') || 'dossier à compléter')}</strong><span class="muted">${esc(categoryLabel(category))} · ${esc(request.customerType || '')} · ${proposalPrice ? `${proposalPrice.toLocaleString('fr-FR')} €` : 'prix à valider'} · ${esc(request.status || '')}</span><div class="result-actions"><button type="button" data-load-request="${esc(request.id)}">Modifier</button>${state.isDirection && /validation|proposition/i.test(request.status || '') ? `<button class="primary" type="button" data-request-decision="Validée" data-id="${esc(request.id)}">Valider pour étude</button><button class="warn" type="button" data-request-decision="À revoir" data-id="${esc(request.id)}">À revoir</button><button class="danger" type="button" data-request-decision="Refusée" data-id="${esc(request.id)}">Refuser</button>` : ''}${state.isDirection ? `<button class="danger" type="button" data-delete-request="${esc(request.id)}">Supprimer</button>` : ''}</div>${request.directionComment ? `<div class="status">Direction : ${esc(request.directionComment)}</div>` : ''}</article>`;
  }
  async function loadRequests() {
    try {
      const result=await api('/api/quote-requests'); const records=result.records || []; $('quoteRequests').innerHTML=records.map(requestCard).join('') || '<div class="muted">Aucune demande enregistrée.</div>';
      $('quoteRequests').querySelectorAll('[data-load-request]').forEach((button)=>button.onclick=()=>{ const request=records.find((item)=>item.id===button.dataset.loadRequest); if(request) fillFromRequest(request); });
      $('quoteRequests').querySelectorAll('[data-request-decision]').forEach((button)=>button.onclick=async()=>{ const comment=prompt('Commentaire de la direction :',button.dataset.requestDecision==='Validée' ? 'Proposition à vérifier puis à transformer en devis.' : '') || ''; try { await api(`/api/quote-requests/${encodeURIComponent(button.dataset.id)}/decision`,{method:'POST',body:JSON.stringify({decision:button.dataset.requestDecision,comment})}); await loadRequests(); } catch(error) { alert(error.message); } });
      $('quoteRequests').querySelectorAll('[data-delete-request]').forEach((button)=>button.onclick=async()=>{ if(!confirm('Supprimer cette demande de devis de la liste active ? Cette action sera tracée.'))return; const reason=prompt('Motif de suppression :','Demande annulée ou doublon') || ''; try { await api(`/api/quote-requests/${encodeURIComponent(button.dataset.deleteRequest)}/decision`,{method:'POST',body:JSON.stringify({decision:'Supprimée',comment:reason})}); if(state.requestId===button.dataset.deleteRequest)newDossier(); await loadRequests(); } catch(error) { alert(error.message); } });
    } catch(error) { $('quoteRequests').innerHTML=`<div class="alert bad">${esc(error.message)}</div>`; }
  }
  ...
  async function init() {
    ...
    checkboxIds.forEach((id)=>$(id)?.addEventListener('change',markChanged));
    ...
    updateCategoryButtons(); updateMetrics(); setSaveStatus('Saisie prête — enregistrement automatique',false); $('jarvisStatus').textContent='Jarvis : de quoi s’agit-il ?'; window.dispatchEvent(new CustomEvent('mavik-quote-change',{detail:payload()}));
  }
  init();
})();
