(() => {
  ...
  function renderDetail(record) {
    ...
    $('detail').innerHTML = `
      ...
      <section class="final"><h3>Clôture de la procédure</h3><p class="meta">Toutes les étapes obligatoires doivent être terminées. L’employé demande ensuite le contrôle final. David ou Bénédicte valide avant la génération du rapport et de la facture.</p><div class="field"><label>Notes finales</label><textarea id="finalNotes" placeholder="Résultat, réserves restantes, recommandations…">${esc(record.finalNotes || '')}</textarea></div><div class="actions"><button id="requestFinal" type="button" ${record.canComplete ? '' : 'disabled'}>Demander le contrôle final</button>${direction && record.procedureStatus === 'Contrôle final à valider' ? '<button class="primary" id="approveFinal" type="button">Valider et terminer l’intervention</button>' : ''}</div></section>`;
