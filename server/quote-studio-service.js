...
function normalizedInput(input = {}) {
  ...
  return {
    ...
    finalPrice,
    targetPrice: number(enriched.targetPrice || standardPrice || finalPrice),
    durationDays: Math.max(1, number(enriched.durationDays || selected?.durationDays || (vehicleType === 'moto' ? 1 : 2))),
    tariffReason,
    standardPriceTtc: standardPrice,
    specialOfferMargin: margin,
    workshopProcedureKey: procedures.get(vehicleType).key,
    workshopProcedure: procedures.snapshot(vehicleType),
    selectedTariff: selected || null
  };
}
...
function confirm(store, input = {}, user = {}) {
  const normalized = normalizedInput(input);
  if (normalized.specialOfferEnabled === true && !['admin', 'associate'].includes(user.role)) throw Object.assign(new Error('SPECIAL_OFFER_DIRECTION_APPROVAL_REQUIRED'), { status: 403 });
  const selected = normalized.selectedTariff;
  ...
  const quote = store.update('quotes', result.quote.id, {
    ...
    tariffKey: normalized.tariffKey,
    service: normalized.service || result.quote.service,
    tariffSource: normalized.tariffReason,
    workshopProcedureKey: normalized.workshopProcedureKey,
    workshopProcedure: normalized.workshopProcedure,
    specialOfferEnabled: normalized.specialOfferEnabled === true,
    specialOfferName: normalized.specialOfferName || '',
    standardPriceTtc: normalized.standardPriceTtc || result.quote.totalTtc,
    discountPercent: margin?.discountPercent || 0,
    discountAmountTtc: margin?.discountAmountTtc || 0,
    directCostTtc: margin?.directCostTtc || 0,
    grossMarginTtc: margin?.grossMarginTtc || 0,
    grossMarginPercent: margin?.grossMarginPercent || 0,
    specialOfferApprovedByDirection: normalized.specialOfferEnabled === true,
    specialOfferApprovedBy: normalized.specialOfferEnabled ? (user.name || user.id || '') : '',
    specialOfferApprovedAt: normalized.specialOfferEnabled ? new Date().toISOString() : '',
    quoteText: exactText,
    mailDraftText: exactText,
