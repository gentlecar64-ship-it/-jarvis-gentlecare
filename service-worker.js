const CACHE='jarvis-gentlecare-v18';
  ...
  if(nav){
    event.respondWith(fetch(event.request,{cache:'no-store'}).then(async response=>{
      ...
      if(url.pathname.endsWith('/employe.html')){
        ...
        finalResponse=new Response(html.replace('</head>','<link rel="stylesheet" href="atelier-responsive.css?v=18"></head>'),{status:response.status,statusText:response.statusText,headers:{'Content-Type':'text/html; charset=utf-8'}});
      }
